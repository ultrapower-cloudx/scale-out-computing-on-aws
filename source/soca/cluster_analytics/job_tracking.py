...
def boto_extra_config():
    aws_solution_user_agent = {"user_agent_extra": "AwsSolution/SO0072/2.7.5.1"}
...
def os_entry_exist(job_uuid, os_index_name):
    ...
    try:
        response = os_client.search(
            index=os_index_name, scroll="2m", size=1000, body=json_to_push
        )
    ...
    while scroll_size > 0:
        ...
        response = os_client.scroll(scroll_id=sid, scroll="2m")
        sid = response["_scroll_id"]
...
def os_index_new_item(body, index):
    add = os_client.index(index=index, body=body)
...
if __name__ == "__main__":
    ...
    client = boto3.client(
        "pricing", region_name="us-east-1", config=boto_extra_config()
    )
        ...
        try:
            for data in values:
                try:
                            ...
                            if used_resources:
                                ...
                                for res in used_resources:
                                    ...
                                    if resource_name == "select":
                                        mpiprocs = re.search(
                                            r"mpiprocs=(\d+)", resource_value
                                        )
                                        if mpiprocs:
                                            job_info["mpiprocs"] = int(
                                                re.search(
                                                    r"mpiprocs=(\d+)", resource_value
                                                ).group(1)
                                            )
                                    ...
                                    if "ppn" in resource_value:
                                        ...
                                        if ppn:
                                            job_info["ppn"] = int(
                                                re.search(
                                                    r"ppn=(\d+)", resource_value
                                                ).group(1)
                                            )

                                    job_info[resource_name] = (
                                        int(resource_value)
                                        if resource_value.isdigit() is True
                                        else resource_value
                                    )
                                ...
                                job_info["simulation_time_minutes"] = float(
                                    job_info["simulation_time_seconds"] / 60
                                )
                                job_info["simulation_time_hours"] = float(
                                    job_info["simulation_time_minutes"] / 60
                                )
                                job_info["simulation_time_days"] = float(
                                    job_info["simulation_time_hours"] / 24
                                )
                                job_info["mem_kb"] = int(
                                ...
                                job_info["job_uuid"] = base64.b64encode(
                                    f"{job_id}_{cluster_id}_{job_info['end_iso']}".encode(
                                        "utf-8"
                                    )
                                ).decode("utf-8")
                    ...
                    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                    print(
                        f"Error with {data} for job id {job_id} - {exc_type} - {fname} - { exc_tb.tb_lineno}"
                    )
                    exit(1)
    ...
    for entry in json_output:
        if os_entry_exist(f"{entry['job_uuid']}", os_index_name) is False:
            if os_index_new_item(json.dumps(entry), os_index_name) is False:
